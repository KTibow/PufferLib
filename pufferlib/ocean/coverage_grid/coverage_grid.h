...
typedef struct {
    ...
    int* actions;                // Required field - 3 discrete actions: GO_FORWARD, GO_LEFT, GO_RIGHT
...
void update_observations(CoverageGrid* env) {
    if (env->obs_type == OBS_TYPE_POSITION) {
        ...
        float max_w = (float)(env->grid_width - 1);
        float max_h = (float)(env->grid_height - 1);
        env->observations[0] = (float)env->x / max_w;           // distance to left wall
        env->observations[1] = (float)(env->grid_width - 1 - env->x) / max_w;  // distance to right wall
        env->observations[2] = (float)env->y / max_h;           // distance to top wall
        env->observations[3] = (float)(env->grid_height - 1 - env->y) / max_h; // distance to bottom wall
    }
}
...
void c_reset(CoverageGrid* env) {
    ...
    env->x = 0;
    env->y = 0;
    env->direction = DIRECTION_NORTH;  // Start facing north
    env->tick = 0;
...
void c_step(CoverageGrid* env) {
    ...
    // Always move forward after processing action
    ...
    if (new_x >= 0 && new_x < env->grid_width &&
        ...
        if (env->visited_count >= env->total_cells) {
            env->rewards[0] += 1.0f;  // Large bonus for complete coverage
            env->terminals[0] = 1;
            add_log(env);
            c_reset(env);
            return;
...
void c_render(CoverageGrid* env) {
    ...
    DrawLine(agent_pixel_x, agent_pixel_y, arrow_end_x, arrow_end_y, (Color){255, 0, 0, 255});
    ...
    snprintf(status_text, sizeof(status_text),
        "Coverage: %d/%d (%.1f%%) | Collisions: %d | Step: %d",
        env->visited_count, env->total_cells, coverage_pct,
