...
void c_reset(CoverageGrid* env) {
    ...
    env->x = 0;
    env->y = 0;
    env->direction = DIRECTION_SOUTH;  // Start facing north
    env->tick = 0;
...
void c_render(CoverageGrid* env) {
    ...
    snprintf(status_text, sizeof(status_text),
        "Pos: (%d,%d) | Dir: %s | Coverage: %d/%d (%.1f%%) | Collisions: %d | Step: %d",
        env->x, env->y, direction_names[env->direction], env->visited_count, env->total_cells, coverage_pct,
