...
int check_dirt_collection(Roomba* env) {
    ...
    float brush_center_x = env->x - (BRUSH_DEPTH / 2.0f) * cosf(env->theta);
    float brush_center_y = env->y - (BRUSH_DEPTH / 2.0f) * sinf(env->theta);
...
void c_step(Roomba* env) {
    ...
    // When near walls, constrain movement to prevent rubbing against walls
    if (near_wall) {
    ...
    // env->rewards[0] += dirt_collected * 0.2f;
...
void c_render(Roomba* env) {
    ...
    // Count active dirt pieces
    int active_dirt = 0;
    for (int i = 0; i < MAX_DIRT_PIECES; i++) {
        if (env->dirt_pieces[i].active) active_dirt++;
    }

    // Draw sensor information
    char sensor_text[300];
    snprintf(sensor_text, sizeof(sensor_text),
        "(%.0f,%.0f) facing %.1f° | %.0f/s, %.0f/s | bump %d%d | Dirt: %d/%d collected: %d",
        env->x, env->y, env->theta * 180.0f / PI,
        env->left_wheel_speed, env->right_wheel_speed,
        env->left_bumper, env->right_bumper,
        MAX_DIRT_PIECES - active_dirt, MAX_DIRT_PIECES, env->dirt_collected_this_episode);
